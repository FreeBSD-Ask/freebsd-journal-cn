...
第三条命令 `RUN yum -y update ...` 更新基础镜像中过时的软件包。下一条命令 `yum -y install httpd` 安装 Apache 的 `httpd` 软件包。命令最后部分 `yum clean` 清除 yum 软件包管理系统维护的所有软件包/仓库元数据，以尽量缩小生成层的大小。同理，yum 命令通过 `nodocs` 标志，在升级和安装软件包时被指示忽略文档。`RUN` 命令的参数由 shell 进程执行，因此构建镜像中所生成层的复杂程度可以相当高。
