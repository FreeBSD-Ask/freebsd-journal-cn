...
在许多服务器上分发水平可扩展的应用，容器是很好的工具。当应用数量和它们的基数增长时，手工管理数量众多的容器会变得困难。

容器编排器是一类应用程序，旨在简化大量容器的管理，隐藏复杂性，提高可靠性，尤其是在自动伸缩和持续部署所造就的动态环境中。在本文中，我们将讨论基于 FreeBSD 的配置，使用 pot（一个支持 Jail 镜像的 Jail 框架）和 nomad（一个由 HashiCorp 开发的与容器无关的编排器）。

## 系统架构

为了解释编排器如何工作，我们需要介绍一些服务并说明它们的角色。

### nomad 客户端

nomad 客户端是一台服务器，接收来自编排器的命令来执行容器。Nomad 客户端也被称为节点，就像 kubernetes 中一样。

在大型部署中，大多数服务器是 nomad 客户端，它们负责执行用户的应用程序。在云原生术语中，nomad 客户端构成集群的数据平面。

Nomad 客户端能支持多种容器驱动：一些驱动与操作系统无关，而其他驱动，像 docker 或 pot，仅在特定操作系统上可用。
...
每个客户端都需要一个服务来下载其被指派执行的容器的镜像。这个需求由容器注册表来满足，它是提供容器镜像的服务。

当编排器选择一个客户端来执行容器时，该客户端将从注册表下载容器镜像，然后启动容器。

在我们的例子中，我们将使用 potluck，一个由 pot 社区维护的公共容器注册表，它从开源的镜像配方目录构建镜像。但是，由于容器镜像包含二进制文件，出于安全考虑，我们强烈建议所有人都拥有自己的本地注册表。

在 pot 中，镜像是通过 fetch(1) 下载的文件，因此注册表可以是一个简单的 web 服务器。

### 服务目录

服务目录是一个附加了额外信息的服务列表，例如实现这些服务的所有容器地址。

当编排器调度实现服务的容器时，它也会将容器地址注册到实现该服务的容器列表中。

服务目录也可以配置为定期检查所有容器的服务健康状态，因此容器地址列表只包含健康的地址。

我们要使用的服务目录基于 consul，一个服务网格应用，同样由 HashiCorp 开发。

### 入口（可选）

由于编排器的动态特性，很难知道服务在哪里运行。每次发生新的部署时，容器都可能被调度到不同的节点和不同的端口。通过入口，我们定义一个代理/负载均衡器，配置为给我们的服务提供固定的入口点。

例如，我们可以这样配置代理，让 URL 路径（例如，**https://example.com/foo**）提供目标服务的信息（即重定向到实现服务 foo 的容器）。另一种常见方法是使用 host header。
...
上述任务节描述了 nomad 调度任务所需的所有细节。任务“nginx-minipot”（1）有一个名为“group1”的组（4），其中有一个名为“www1”的任务（9）。

任务“www1”是一个 pot 容器（10），镜像注册表是 potluck（23），pot 镜像是 nginx（24），版本是 1.1.13（25）。指定任务基于 pot 驱动后，编排器会将这个任务调度到支持 pot 的客户端。在我们的例子中，服务器和客户端是同一台机器。

与通常使用 rc 脚本引导的 Jail 相比，我们将直接执行 nginx（26），不需要任何额外的服务。args 参数（27）很重要，它让 nomad 能正确跟随容器生命周期并捕获其日志。Pot 将负责初始化网络和一切所需内容。

port_map 节（28）和网络节（6）表示 nginx 将在 Jail 中监听 80 端口，但 nomad 客户端将使用不同的端口（“http”），由 nomad 服务器动态分配。

服务节（11）提供了 nomad 用于将服务注册到 consul 的信息。在我们的例子中，任务“www1”实现了服务“hello-web”（11），它将使用 nomad 服务器分配的端口“http”（14）注册到 consul。对于 IP 地址，nomad 服务器将使用在调度期间确定的 nomad 客户端 IP 地址。

在我们的例子中，任务还配置了一个 tcp 健康检查，consul 将每 5 秒运行一次该检查，以确定实例的健康状态。

这个任务描述需要保存为一个文件（即 nginx.job），任何用户都可以通过命令启动服务。
...
Minipot 是单节点安装，适合作试验场，用于学习或本地测试。
...
我想强调走到这一步所需要的社区努力，从第一位 nomad-pot-driver 开发者 Esteban Barrios，到 Michael Gmelin（grembo@），他确实为提升该解决方案的可靠性和稳定性提供了大量帮助。我还想提及 Stephan Lichtenauer 和 Bretton Vine，他们参与了 Potluck（公共镜像注册表）以及许多其他项目，如 Ansible playbook 和专注于 pot 与 nomad 用例的[博客文章](https://honeyguide.eu/tags/pot)。

---

**LUCA PIZZAMIGLIO** 是 FreeBSD 项目的 port 提交者，也是 port 管理团队（portmgr）的成员。2017 年，他启动了 pot 项目，旨在探索 FreeBSD 上的容器会是什么样子。
