...
我第一次见到 Hans 是在 2015 年，当时他正在为 Mellanox 网卡开发 `mce(4)` 驱动程序。我们合作使 `mce(4)` 驱动程序成为 FreeBSD 中性能最高的网卡驱动程序之一。正是在这段时间里，我了解到了他的才华。他经常有一些听起来"疯狂"的想法，但实际上却非常出色。其中一个例子是他提出使用网卡提供的 RSS 流标识符对传入的 TCP 数据包进行排序，以便将来自同一 TCP 连接的所有数据包连续地交给 LRO。这个想法最初被我视为不切实际而否定，但对于 Netflix 能够实现从单台服务器提供 100Gb/s 视频流量的性能目标至关重要，并且持续为 Netflix 节省大量的 CPU 资源。
