...
**Vector Packet Process (VPP)** 是高性能框架，用于在用户空间处理数据包。得益于 FreeBSD 基金会和 RGNets 的项目，我获得资助将 VPP 移植到 FreeBSD，并且非常高兴与《FreeBSD 期刊》的读者分享一些基本的使用方法。
...
在我们的示例中，我们将使用 VPP 与 Netmap，Netmap 是高性能的用户空间网络框架，作为 FreeBSD 的默认组件提供。使用 Netmap 之前，接口需要进行一些配置——接口需要处于启用状态，并且需要配置`promisc`选项。
...
VPP 是一款令人印象深刻的软件——解决了兼容性问题后，VPP 的核心部分就相对容易移植。即使仅做最小调优，VPP 在 FreeBSD 上与 netmap 配合使用时也能够达到相当出色的性能，而如果配置了 DPDK，它的表现会更好。VPP 的文档正在逐步增加关于在 FreeBSD 上运行的信息，但开发者确实需要更多关于 FreeBSD 上使用 VPP 的使用案例。
