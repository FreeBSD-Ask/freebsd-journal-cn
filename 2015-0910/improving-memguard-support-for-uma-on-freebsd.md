...
- 在 `lock_init()` 中使用 LO_NEW 标志。锁初始化时可用 `_NEW` 标志实现。`mtx_init()` 使用 MTX_NEW 标志。`rm_init()` 使用 RM_NEW 标志。`rw_init()` 使用 RW_NEW 标志。`sx_init()` 使用 SX_NEW 标志。
...
Luke Chang-Hsien Tsai（蔡易修）毕业于台湾国立交通大学，获计算机科学硕士学位。他自 2012 年起成为 FreeBSD 开发者。工作中他开发新功能并改进 ZFS 性能。业余时间他向 FreeBSD 项目提交补丁，主要在 DTrace 与安全领域。
