...
无论是升级基础系统还是软件包，FreeBSD 上有几个目录至关重要。FreeBSD 的核心程序二进制位于 **/bin**、**/sbin**、**/usr/bin** 与 **/usr/sbin**。得益于 **/usr** 数据集不挂载，这些目录现在位于根数据集上。软件包位于 **/usr/local** 之下——但那里也在根数据集上。**/var/db** 目录包含软件包数据库、freebsd-update 记录乃至 locate 数据库。因为 **/var** 未挂载，这些关键的 **/var/db** 文件现在也是根数据集的一部分。
