...
答案是内核端 TLS，简称 kTLS，将 TLS 与新的 sendfile 模型结合。这种混合 TLS 方案（由 John Baldwin 在 vBSDCon 2019 大会上描述）将会话管理保留在应用程序空间，并将批量加密插入到内核中的 sendfile 数据管道。TLS 会话协商和密钥交换消息从 Nginx 传递到 TLS 库，会话状态存储在该库的应用程序空间。一旦 TLS 会话建立并与客户端交换生成适当的密钥，这些密钥就与客户端的通信套接字关联，并共享到内核。
