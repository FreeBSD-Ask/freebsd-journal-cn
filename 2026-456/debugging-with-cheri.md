...
连接到客户机后，我向上回溯栈帧到 `make_packet()` 函数的栈帧。查看图 5 中传给 `bcopy()` 的参数，代码试图从仅 4 字节长的缓冲区复制 12 字节。阅读 `bcopy()` 周围循环中的其余代码，可以看出这些 tag 结构有一个 4 字节的首部，包含类型和长度（以网络字节序表示），其后是数据缓冲区，长度由首部给出。`tlen` 变量包含总长度，那么为何指针的边界过小？
