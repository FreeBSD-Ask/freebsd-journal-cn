...
FreeBSD 相比 Linux 有何优势？这是我常被问到的问题，却难以回答，因为 FreeBSD 有太多方面做得很好。从何说起？你可以滔滔不绝地讲它健壮的网络栈，或 DTrace 和 Capsicum 等前沿技术。然而我认为 FreeBSD 真正出彩的领域是存储。开发者不仅花大量时间确保你写入磁盘的 0 和 1 以正确顺序落盘，还要尽可能快地完成。不仅如此，他们还要确保这些 0 和 1 以正确的顺序、尽可能快地读回。无论你只是在笔记本上归档家庭照片，还是向网络上数千客户端提供多 PB 的 ZFS 卷，FreeBSD 都提供了健壮可靠的平台来满足你的存储需求。
...
按路径打开时不要在 `vdev_geom_attach` 中检查 `SPA_MINDEVSIZE`。它与 `vdev_open` 中的检查重复，且在此处附加失败会打印错误的错误消息。
