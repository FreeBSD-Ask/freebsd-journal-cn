...
找出哪个应用程序在产生所有 I/O 的最快方法之一是使用 top。FreeBSD 的 top 有 `-m` 标志用于切换模式。在 IO 模式下，它不再按 CPU 和内存使用情况跟踪应用程序，而是按读、写和其他 IO 操作跟踪。这有助于判断哪个应用程序在消耗所有 IO 资源。要进一步细分，请参见下文 DTrace Toolkit 部分。
