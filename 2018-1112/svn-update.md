...
当 **rc.conf(5)** 中的变量 `service_delete_empty` 设置为“YES”（默认为“NO”）时，使用 `service $foo delete` 修改后，如果 **/etc/** 或 **/usr/local/etc** 中的 rc.conf.d 文件为空，则会被删除。
...
主动 PS/2 多路复用是一种将最多四个 PS/2 指点设备连接到计算机的方法。启用多路复用模式允许使用路由前缀将命令定向到各个设备。多路复用模式报告输入，每个字节都标记以识别其来源。此方法与 **psm(4)** 当前支持的方法不同，后者将所谓的客户设备（trackpoint）连接到位于主机设备（touchpad）上的特殊接口，稍后将客户协议转换为特殊封装数据包格式。
