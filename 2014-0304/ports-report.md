...
由于第一个分支——2014Q1——是实验性的，你可能还没有听说过它。2014 年 1 月发布了第一个季度分支，旨在提供一个稳定且高质量的 Ports 树。这些稳定分支是每三个月对 head Ports 树做一次快照，目前支持期为三个月，在此期间会接收安全修复以及构建和运行时修复。包会定期在该分支上（每周）构建并通过 pkg.FreeBSD.org（/quarterly，而非通常的 /latest）发布。4 月 1 日（不开玩笑），2014Q2 分支创建，首批构建随后不久开始。
