...
克隆操作会下载整个代码库。有两种下载方式。大多数用户会选择完整克隆（deep clone）代码库，但在某些情况下，你可能希望执行浅克隆（shallow clone）。  
...
如果你对历史记录感兴趣、计划进行本地修改，或者需要在多个分支上工作，完整克隆是最佳选择。此外，这种方式也最容易保持代码库的最新状态。如果你只对某个分支的历史感兴趣，并且磁盘空间有限，可以使用参数 `--single-branch` ，仅下载指定分支的历史记录（但某些合并提交可能不会引用被合并的分支，这对某些需要详细历史记录的用户来说可能很重要）。  
...
此命令会更新代码库。在 Git 中，快进合并（fast forward merge）仅需移动分支指针，而不会重新创建提交记录。始终执行快进合并/拉取（pull）可以确保你的代码库与 FreeBSD 官方代码库保持完全一致。如果你需要维护本地补丁，这一点尤为重要。  
