...
ZFS 中存储的池化特性意味着所有可用空闲空间对每个数据集都可用。与传统的把 RAID 卷分区或创建独立卷的做法相比，空闲空间不会碎片化。缺点是单个工作负载或用户可能耗尽所有可用空间。虽然可以用配额解决，但并不总能解决问题。ZFS 还提供预留系统，可以为关键数据库或安全日志等特定数据集保留最低限度的空间，对其他任何数据集不可用。为确保电子商务数据库不会因 HTTP 日志而耗尽空间，给它的数据集预留。

虽然近期 ZFS 的改进已大大改善情况，但接近满的 ZFS 存储池性能会很差。如果存储池完全满了，解决该情况的命令可能耗时极长。应对方法是创建名为“reserved”的新数据集，预留存储池总容量的 20% 到 25%。这可防止最后那一点关键空间被耗尽。预留可以放宽，让管理员执行必要操作或维持系统运转，直到存储池可以扩容。
...
希望这些技巧和最佳实践对你有所帮助，帮你避免一些 ZFS 新手常犯的错误。我们希望看到你加入蓬勃发展的 OpenZFS 用户和开发者社区。特别感谢备份大师 Dan Langille，以及 Michael Dexter——他为各种客户包扎过许多自己造成的脚伤，并分享了所学。
