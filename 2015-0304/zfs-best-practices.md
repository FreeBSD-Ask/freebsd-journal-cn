...
硬件 RAID 控制器还可能引发无数其他问题，使用更便宜的简单 HBA 控制器反而好得多。硬件 RAID 控制器有时会掩盖某些错误并静默重试命令，对 ZFS 隐藏这些信息。这主要是过去的遗留做法，那时操作系统无法被信任能优雅处理错误。没有这些信息，ZFS 无法做出明智决策，也无法通过从奇偶校验位置读取数据来自我修复故障盘。
