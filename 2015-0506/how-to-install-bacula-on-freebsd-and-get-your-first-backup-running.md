...
坦白说：我有偏好。我喜欢 PostgreSQL、FreeBSD、ZFS 与 Bacula 的组合。事实上，我太喜欢它了，以至于为 Bacula 编写了 PostgreSQL 后端。从那以后，我使用 Bacula 已超过 10 年。我备份到 HDD、DDS、DLT 与 SDLT。Bacula 已被证明可靠，它就是能用。客户端覆盖了广泛的操作系统，Bacula 也不在乎你用的是什么文件系统。

我也是 Bacula 的 FreeBSD port 维护者。

我最初为 O'Reilly 撰写过关于 Bacula 的文章。后来我把那篇文章转载到了我的日记里。我为本期 FreeBSD Journal 更新了那篇文章。
