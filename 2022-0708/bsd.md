...
我与 **Front Range BSD 用户组** 合作，连续几年在 **SCaLE**（Southern California Linux Expo）上教授《FreeBSD 入门》（Getting Started with FreeBSD）研讨会。我之所以这样做，是因为我深知 FreeBSD 工具的强大，也乐于通过研讨会分享自己的经验。
...
学员们陆续走进会场，映入眼帘的是投影屏幕上《FreeBSD 入门》的标题幻灯片——由 FreeBSD 基金会的市场协调员 **Drew Gurkowski** 设计。这次演示还通过 YouTube 直播，链接如下：  
📺 [观看直播](https://www.youtube.com/watch?v=ByFCRwMJATM)  

往年，研讨会通常由 FreeBSD 基金会的执行董事 **Deb Goodkin** 做开场介绍，而这一次，**Drew Gurkowski** 出色完成这一任务。  
...
在这一阶段，我们会短暂休息，我会利用这段时间帮助尚未跟上的学员。  

## 常见问题与解决方案

在研讨会中，我遇到的最常见问题主要与虚拟机的错误配置有关。例如：  
...
接下来的步骤是演示服务器与桌面之间的界限有多么模糊。只需安装几个软件包并修改一些配置文件，桌面环境就能准备就绪。我们只需运行 `startx` 命令，就可以让 FreeBSD 启动图形桌面。  
...
在窗口管理器运行后，学员们可以与 GUI 应用程序交互，例如网页浏览器、编程 IDE、文件管理器等，进一步体验 FreeBSD 作为桌面操作系统的能力。  
...
研讨会中，我们还提供了 **Ansible Playbook**，用于在远程服务器上搭建 Poudriere，构建所有自定义软件包，并将编译好的文件同步回本地：
...
这次的学员热情高涨，有些人甚至专门带来了笔记本电脑，计划在现场安装 FreeBSD。其中，一些人对 FreeBSD 的 WiFi 配置颇感兴趣。对于这类问题，我们提供了一些简单的方法：  
...
- **内部 WiFi 配置**：可通过 **/etc/rc.conf** 和 **/etc/wpa_supplicant.conf** 配置，详细选项请参考 **FreeBSD 手册页第 5 节**。  
