...
VirtIO PCI 驱动创建一个模拟的 PCI 设备，然后提供给虚拟机。模拟的 PCI 设备使用 virtqueue 直接访问分配给设备的内存，从而提升虚拟化环境中的性能。
...
非映射 I/O 支持最初在 r248508 中为 `ahci` 和 `sdhci` 驱动引入。后续修订为其他驱动提供了支持。

## Raspberry Pi 和 BeagleBone 支持

FreeBSD 10 可在 Raspberry Pi、BeagleBone 和若干其他嵌入式平台上运行。虽然 FreeBSD 项目尚未为这些平台提供官方镜像，但已有若干工具集可创建可写入 compact flash 卡的镜像。

这些工具之一是"Crochet"，可用于为 Raspberry Pi、BeagleBone 和若干其他平台构建镜像。Crochet 可在 <https://github.com/kientzle/crochet-freebsd> 找到。

Raspberry Pi 支持在 r239922 中引入。
