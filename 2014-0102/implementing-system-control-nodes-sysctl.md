...
在内核中，`<sys/sysctl.h>` 头文件提供若干宏来声明控制节点。每个声明包含父节点名、分配给此节点的编号、节点名、控制节点行为的标志，以及节点描述。某些声明需要额外参数。父节点用其全名标识，但以单下划线为前缀，点替换为下划线。例如，“foo.bar”父节点用“_foo_bar”标识。要声明顶层节点，使用空父名。编号应使用 `OID_AUTO` 宏请求系统分配唯一编号。（某些节点因历史原因使用硬编码编号，但所有新节点应使用系统分配的编号。）flags 参数必须指明节点支持的访问类型（读、写或两者），也可包含其他可选标志。描述应是简短描述节点的字符串。当向 `sysctl.8` 传递 `-d` 标志时，会显示此描述而非值。
...
具有对应可调参数的系统控制节点应使用 `CTLFLAG_RDTUN` 或 `CTLFLAG_RWTUN` 标志指定节点允许的访问。注意这不会导致系统根据节点名隐式获取可调参数。可调参数必须显式获取。但它确实向 `sysctl.8` 工具提供提示，用于诊断消息。
