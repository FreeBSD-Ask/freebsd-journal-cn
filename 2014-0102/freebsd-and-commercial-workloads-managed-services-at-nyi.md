...
NYI 还在内部使用 jails 来实现硬件高效的域名系统（DNS）服务器。例如，一台 FreeBSD 机器可能包含一个用于递归 DNS 的 jail、一个用于权威客户 DNS 的第二个 jail 和一个用于权威 NYI DNS 的第三个 jail。然后，这些 jail 中的每个 DNS 服务器在机器集群内使用 CARP 确保故障转移冗余。图 5 展示了这一点。
