...
可以在 DSs 中配置镜像，这样 MDS 文件的数据文件将镜像在两个或更多 DSs 上。使用此功能时，DS 故障不会停止 pNFS 服务，并且故障的 DS 可以在修复后恢复，同时 pNFS 服务继续运行。虽然双向镜像是常态，但可以设置最多四或 DSs 数量（取较小者）的镜像级别。镜像级别指的是在不同 DSs 上保留多少份数据文件。FreeBSD MDS 仍然是单点故障，就像普通 NFS 服务器一样。

Plan B 概览
一个 Plan B pNFS 服务由单个 MDS 和 K 个 DSs 组成，全部是 FreeBSD12 系统。客户端将像挂载普通 NFS 服务器一样挂载 MDS。创建文件时，MDS 创建一个与普通 NFS 服务器创建的相同的文件树，不同之处在于所有常规（VREG）文件都为空。因此，如果直接在 MDS 服务器上（而不是通过 NFS 挂载）查看 MDS 上导出的树，所有文件大小都为 0。这些文件中的每一个还将具有系统属性名称空间中的两个扩展属性：
...
pNFS 服务位于 FreeBSD-current 中，将在 FreeBSD12 发布时包含在其中。在 FreeBSD12 发布之前，可以使用 FreeBSD-current 快照分发进行测试。

设置使用 Plan B 的 FreeBSD pNFS 服务器
让我们假设有五台 FreeBSD12 系统，其中四台配置为 DSs，使用双向镜像和 AUTH_SYS 进行示例。
...
并且 **/data0**、**/data1**、**/data2** 和 **/data3** 目录需要在 MDS 上创建用于挂载 DS 数据文件。注意，"soft,retrans=2" 通常不用于 NFSv4 挂载，但这是一个例外，因为 DS 上不执行状态操作。这样做允许代理操作到 DS 失败，并在发生时禁用失败的 DS。
...
1. 元数据服务器（MDS）在尝试在 DS 上执行代理操作时检测到问题。这就是为什么 DS 服务器在 MDS 上以 "soft,retrans=2" 选项挂载。这可能需要几分钟才能在 DS 故障或网络分区发生后触发。
...
（"-N" 选项确保即使线程卡住尝试在失败的 nfsv4-data2 上执行 RPC，umount 也能工作。）
...
修复 nfsv4-data2 的第一步是确保 nfsv4-data2 上的文件过期副本在 nfsv4-data2 重新上线时不被使用。为此，我们转到 MDS 上的 **/export** 目录，并通过以下命令将 nfsv4-data2 替换为 IP 地址 0.0.0.0：
```sh
...
Rick Macklem 与 BSD 打交道已太久。他 1985 年首次对 BSD 的贡献是将 4.2BSD 移植到 MicroVAXII。之后不久，他贡献了成为 4.3BSD Reno 一部分的第一个 NFS 实现。他在一所加拿大大学担任系统管理员三十年，现已退休，仍在为 FreeBSD 工作 NFS。是的，这篇文章是用 "ed" 撰写的，"ed" 仍是他选择的编辑器。
...
已参与？
不要忘记查看最新的资助机会，请访问 freebsdfoundation.org

加入我们！

FreeBSD 在国际上被公认为创新领导者，提供高性能、安全、稳定的操作系统。

FreeBSD 不仅易于安装，而且可运行大量完整源代码。

为像你这样的人继续开发这个健壮的操作系统。
