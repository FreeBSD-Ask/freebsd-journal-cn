...
在 FreeBSD 中，NVMe 控制器和命名空间通过 nvme(4) 驱动程序枚举和初始化。nvme(4) 还负责提供接口，将这些命名空间作为块设备公开，但不直接向 GEOM 或 CAM 注册这些命名空间。nvd(4) 将每个命名空间向 GEOM 注册为块设备。自 nvd(4) 原始开发以来 NVMe 已成为主流，Netflix 添加了 nda(4) 作为 nvd(4) 的替代方案。nvd(4) 驱动程序是 NVMe 协议之上非常薄的一层，旨在高事务速率下运行。nda(4) 驱动程序与 CAM 集成，包括其错误恢复和高级排队。它还通过 I/O 调度器向驱动器提供流量整形，以提升整体性能。
     ...
     NVMe 系统集成
         ...
         nvd(4) 直接与磁盘层集成。
                                                                                          文件系统
           这有利于将命
                                                                        缓冲区缓存        令排队到 NVMe 驱动器，延迟尽可能
           小。由于 NVMe 驱动器设计为                 GEOM
            扩展，这相当有效。获取                                      磁盘接口
          I/O 到驱动器的最小延迟意味着                              nda
          更低的延迟和更简单的代码路径。                  nvd            CAM
               相比之下，nda(4) 驱动程序通过                                                                     nvme_sim
          CAM 连接。CAM 调度 I/O 到

36 FreeBSD Journal
驱动器。这可能涉及重新排序命令。CAM 系统设计为接受高队列
...
流受益于偏向读取操作。许多 NVMe 驱动器在 TRIM 方面存在问题，但很快将能够大量节流或整形到驱动器的修剪。nda(4) 能够将多个修剪
合并到驱动器的一个修剪请求中。
...
在本文稍后描述。
  ...
  完成
队列是一个 sepa-                    696     /* 将命令从追踪器复制到提交队列。 */
rate 连续主机                    697     memcpy(&qpair->cmd[qpair->sq_tail], &req->cmd, sizeof(req->cmd));
内存区域，
                    698
16 字节条目。
...
通过填写
                    702     wmb();
中的下一个条目
                    703     nvme_mmio_write_4(qpair->ctrlr, doorbell[qpair->id].sq_tdbl,完成队列。此条目
                    704         qpair->sq_tail);包含提交
队列的 ID 和主机提交命令时使用的每个队列的命令 ID。然后
控制器中断主机，驱动程序可以开始处理完成条目。NVMe 在每个完成队列条目中定义一个
相位位，使主机能够确定哪些完成条目
有效。控制器第一次通过队列时会将此相位位写入 1，然后
在随后的队列遍历中在 0 和 1 之间交替。驱动程序根据期望值检查每个完成队列
条目的相位位，以确定哪些条目包含新的完成。在
nvme(4) 中，这在 nvme_qpair_process_completions() 中完成：
...
• 命名空间标识符——主要用于 I/O 命令，但某些 admin 命令如 IDENTIFY
  也使用此字段；注意这推断任何 I/O 队列并提交 I/O 到任何命名空间
• 两个物理区域页面（PRP）条目
...
I/O 栈始终使用虚拟连续缓冲区，这始终可以由 PRP 表示。
  ...
  其他 PRP 条目的列表。但这
  意味着对于大型 I/O，我们需要为映射
  进行 DMA 的 PRP 列表提供缓冲区。

nvme_request 和 nvme_tracker
PRP 列表有两个注意事项：
• PRP 列表缓冲区需要为 DMA 映射，因此 PRP 列表缓冲区在
  分配 qpair 时分配和映射。这避免了在性能路径中映射 PRP 列表。
• PRP 列表缓冲区在某些环境中可能变得很大。PRP 列表缓冲区的大小与
  MAXPHYS 成正比。FreeBSD 默认 MAXPHYS 为 128KiB，导致最大 PRP 列表为 256 字节。Netflix
  但部署时 MAXPHYS 为 1MiB，导致 2KiB PRP 列表缓冲区大小。因此，我们需要 jud-i
  cious 决定每个 qpair 分配多少 PRP 列表。nvme 驱动程序自动将请求大小限制为    •
  MAXPHYS 和 2MiB 中较小者，这是适合 SG 列表可用的两个 PRP 页面的最大缓冲区。
  nvme(4) 定义两种不同的结构——struct nvme_request 和 struct nvme_tracker。
  当分配 I/O qpair 时，分配 1285 个 nvme_trackers，以及每个
...
注意 nvme_command 是一个 64 字节的提交队列条目，但此时我们只是准备


                                                                                  July/August 2018  39
  提交队列条目——它稍后将被复制到实际提交队列中。最后，我们调用
...
40  FreeBSD Journal
    ...
    总结
     ...
     NVMe 能力。下次你看 Netflix 时，希望你能对这些
       ...
       Jim Harris 是 Intel 数据中心      Warner Losh 是 Netflix 的高级软件工程师，
         小组的主要软件工程师，于      负责 FreeBSD 存储系统
      2011 年获得 FreeBSD 源代码提交权限。他是      的视频交付服务器优化。他作为
        FreeBSD nvme 和 nvd 驱动程序的       FreeBSD 贡献者已超过 20 年，目前
       原始作者       也是 nvmecontrol 管理      在 FreeBSD 核心团队任第六届任期。Warner 改进了很多系
          实用程序。Jim 还帮助将库和      统——最近改进了 FreeBSD 的引导加载程序。
        工具（如 DPDK 和 Intel VTune Amplifier）      加入 Netflix 之前，他生产闪存驱动器并
      带到 FreeBSD，并将 FreeBSD nvme 驱动程序      为准确性测量原子钟。他的代码
       移植到用户空间，作为他目前担任存储      仍在测量一些创建 UTC 的时钟！
      性能开发套件软件架构师的角色。
SUBSCRIBE TO DAY

            J O U R N A L®TM
                          访问 www.freebsdfoundation.org
                               1 年 19.99 美元/单本 6.99 美元
