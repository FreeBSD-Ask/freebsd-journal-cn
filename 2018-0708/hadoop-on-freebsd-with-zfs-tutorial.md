...
注意，此 playbook 不使用 FreeBSD 的 Hadoop port/package 默认路径。这样，在 port 更新之前就可以使用更高版本的 Hadoop。默认的 FreeBSD 路径在需要时可以轻松替换。本教程中介绍的配置文件只包含使基本 Hadoop 设置运行所需的最少部分。FreeBSD port/package 包含的示例配置文件有比最初需要更多的配置选项。然而，port 是一旦集群搭建完成，扩展和学习 Hadoop 集群的绝佳资源。
