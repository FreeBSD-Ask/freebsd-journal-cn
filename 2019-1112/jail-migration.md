...
系统中时不时需要把东西挪来挪去。通常是为了给其他东西腾地方，或者新位置更合适。简单的文件和目录如此，ZFS 数据集如此，有时整个安装也是如此。我的情况是一个 jail，最初作为测试系统起步，我很喜欢它的安装，不想在原来那台宿主机改作他用时从头再搭一遍。幸好我用 iocage 作为管理框架搭建了这个 jail。我知道它支持迁移，但从未用过。所以这正好是个机会，把我的学习整理成一篇文章。
...
iocage 安装时，在专用 pool 上创建了一个名为 `images` 的数据集，以及其他数据集。运行迁移命令时，构成 jail 的归档和校验和会放到 `images` 目录中：
