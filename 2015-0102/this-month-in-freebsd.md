...
当然，这是假设没有出问题。出问题时必须处理。问题可能各不相同，不一定表示 FreeBSD 源码树有问题。也有人为因素。不止一次（次数比我愿意承认的多），我不得不重启 BETA、RC，甚至最终的 RELEASE 构建，因为我自己犯了错误。这些错误可能是在构建所用的 `release.conf` 配置文件中为源码树设置了错误的 Subversion revision，也可能是配置文件中一个细微的拼写错误。错误难免会发生，一旦发生，就意味着一个不眠之夜。
