...
本系列文章将讲述在 FreeBSD 上开发 WiFi 的路径。关于驱动开发的底层细节以及 LLVM 编译优化等内容，已经有很多优秀的资料。本篇作为三篇文章中的第一篇，将解释相关术语，并演示如何在 FreeBSD 上配置 WLAN 接口用于测试。这个最小化配置足以开始发现问题。后续文章将讨论 WiFi 驱动的工作原理，以及它们如何与 FreeBSD 中更大的 `net80211` 协议栈交互。
