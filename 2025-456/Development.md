...
本系列文章将讲述在 FreeBSD 上开发 WiFi 的路径。关于驱动开发的底层细节和 LLVM 编译优化等内容，已经有很多优秀的资料。本篇作为三篇文章中的第一篇，将解释相关术语，并演示如何在 FreeBSD 上配置 WLAN 接口用于测试。这个最小化配置足以开始发现问题。后续文章将讨论 WiFi 驱动的工作原理，和它们如何与 FreeBSD 中更大的 `net80211` 协议栈交互。
...
| Channel（信道） | 一个射频频率和它的参数，有时与“频段”可互换使用。 |
...
大多数家庭中最常见的 WiFi 网络是一种接入点（AP），它作为站点（或客户端）通往更大互联网的网关。在许多部署场景中，很可能就是你的家中，接入点通常是一个路由器，负责将客户端的流量转发到互联网（可能通过调制解调器）、分配地址和执行其他大量网络任务。
...
在 FreeBSD 上开发 WiFi 时，我们需要了解硬件和 `net80211` 层分别承担的任务。上面是简短的总结，实际上还有许多其他状态和认证模式。`IEEE80211` 标准已接近 30 年历史，拥有丰富的发展历程。
...
- `bmiss 10 scanvalid 60 bgscan bgscanintvl 300 bgscanidle 250 roam:rssi 7 roam:rate 1 wme bintval 0`
...
现在我们已经拥有了构建纯 FreeBSD AP 站点并调试无线流量的所有条件，接下来讨论如何测试。

我们可以利用 AP 站点和监控模式来验证和调查关联过程中和数据发送时的无线包。
...
本文介绍了在 FreeBSD 上开始开发 WiFi 所需的背景知识和术语，但还未涉及任何代码。使用这里的示例搭建测试网络是 WiFi 开发的核心部分，如果你迫不及待，不想等待第二部分，我相信只要配备足够硬件测试，你就能开始发现问题。

下一篇文章中，我们将探讨 WiFi 驱动的生命周期，它需要具备的核心功能，和与 `net80211` 交互的接口，这些接口能够承担驱动的大量工作。
