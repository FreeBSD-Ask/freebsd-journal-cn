...
它由一个基于 PXE 的设置组成，可针对传统的 PXE 环境和更现代的、基于 UEFI 的网络引导调整。我将此引导序列与一台 TFTP 服务器（由 `inetd` 管理）、DHCP 服务（使用 `isc-dhcp44-server` 软件包）以及只读的 NFS 挂载结合在一起。完整描述该设置超出了本文的范围，但归根结底，FreeBSD 安装器的可用性也取决于其开发环境的便利性。你可以根据自己的需求自由调整以下配置片段。
