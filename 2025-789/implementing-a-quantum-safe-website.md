...
在撰写本文时，OpenSSL 3.5.0 已发布，带有原生的 PQC 算法支持，但 pkg(8) 的说明指出它处于 beta 阶段，不适合生产环境。因此，在余下实现中，我将坚持使用 OpenSSL 3.4.1。
...
接下来我们将配置 nginx，确保启用了 **HTTP_SSL**（它默认应该是开启的，但最好还是确认一下）。我不会调整其他任何设置。
