...
很久以前，我开始玩 Home Assistant，对自己做出的几个自动化很自豪。浴室里有一个运动传感器，可以打开灯。如果太阳已经落山，灯会以非常暗的橙色亮起；否则，它会以最大亮度并使用白光。还有一个自动化，当相对湿度超过预设值时，自动开启除湿机。另外还有一个安全措施。不幸的是，我们有一扇地下室窗户，如果从南边下雨，就容易被淹（虽然这种情况比较少见）。我安装了一个外部和一个内部的湿度传感器；当它们被触发时，我们会在 Telegram 上收到一条消息，并能据此采取措施。整个系统托管在一个运行 HAOS 的 bhyve 虚拟机中。

## 需求

在阳台和花园里有两个无线电控温度计，我希望能有一个简单的总览，更重要的是一张清晰的室外温度曲线图。这些设备很简单，定期通过 433MHz 无线电发送数据。研究之后发现，用一个标准的软件无线电（SDR）适配器就能监听这些信号并进一步处理。将它与优秀的 rtl_433 和 rtl-sdr 项目结合，就是答案。

把一个基于 RTL2832 的接收棒插到笔记本电脑上后，我开始收到温度计的数据，同时也收到了邻居的设备，甚至是路过汽车的胎压监测系统（TPMS）的数据。

好在 rtl_433 软件已经支持将数据发布到 MQTT 服务器。Home Assistant 里本来就有一个用于 ZigBee 网络的 MQTT 服务，所以只需要创建一个新用户来发布数据即可。
...
主机架位于地下室，因此那里的无线信号很差。我把 SDR 放在一楼，并将其连接到现有的 pfSense 路由器。这需要手动安装软件包，并费力地让它在启动时运行。一个务实的解决方案是启动一个 screen 会话，在其中运行 rtl_433。
...
2019 年在挪威举办的 EuroBSDCon 上，Tom Jones 做了一个题为《使用 FreeBSD 进行硬件黑客入门》的教程。每个参与者都收到了一只小盒子，里面有开关、LED、电缆和其他组件，最重要的是一个 Arm SBC。

那是一块 FriendlyELEC 的 NanoPi NEOLTS。会议结束后，我用了一段时间，但现在它主要被放在抽屉里。所以，我开始评估它作为运行 rtl_433 并获取无线数据的平台。

安装的软件已经过时，所以我买了一张新的 MicroSD 卡，并按照 Tom Jones 页面上发布的说明，成功把采集系统迁移到了上面。([https://adventurist.me/posts/00297](https://adventurist.me/posts/00297))

一开始一切运行正常，也很有趣，但几天后系统突然停止工作，无法通过 SSH 访问，所以我只好给它断电重启。最好是连接一个串口控制台来找出问题所在，但当时我没时间去正确排查。

怀疑是与操作系统版本存在某些不兼容，我后来写了一个粗糙的 shell 脚本，基于 Tom 的说明来自动化镜像创建。运行了几次之后，我开始把它参数化，以便更快地更换版本，并添加了一些其它实用的小功能。
...
JSON 输出也会写入 `/var/log` 中的一个 JSON 文件。

## 经验教训

rtl_433 配置文件中的最后一行才是关键。在我的原型系统里，日志被写入 `/tmp`，大约 36 小时就会填满，从而导致系统挂起。所以，如果你不打算定期清理，千万不要把日志写到 `/tmp`。

硬件必须可靠。我在 Pine A64+ 上遇到很多问题，尤其是它的重启问题。
...
未来，我希望探索使用 **nanobsd** 进行开发的可能性，并考虑这个项目是否可以作为深入研究 **Crochet/Poudriere 镜像创建**的一个好机会。

---

**Vanja Cvelbar** 是意大利国家研究委员会材料车间研究所的技术合作者，担任 IT 组负责人，负责管理支撑科研与运行的 IT 基础设施与服务。他的专长涵盖系统管理、网络以及开源技术。在以 Linux 为主的工作二十年后，他又回归了 FreeBSD —— 他的第一个系统是 3.2 版。如今，他专注于部署和维护基于 FreeBSD 的环境，为研究所提供核心 IT 服务。
