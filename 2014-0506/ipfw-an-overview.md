...
这会用一些便于重现由网络问题引起的应用行为的选项重新配置管道 30003。选项 `delay` 给每个数据包增加 150 毫秒延迟，模拟洛杉矶到伦敦之间的延迟。选项 `burst` 允许略高于最大带宽的瞬时使用——前提是管道此前未满；若管道空闲，则前 128KB 数据不经速率限制通过。下一个选项模拟 0.1% 的丢包率（`plr`），引发偶发重传，正如在不甚理想的网络上常见的那样。选项 `queue` 设定在拒绝后续数据包之前，可接受的最大额外数据量（以数据包或 KB 为单位）；通过设置低速率上限配大队列，可借此模拟缓冲膨胀。最后一个选项 `noerror` 使防火墙在丢包时不向调用应用返回错误。
