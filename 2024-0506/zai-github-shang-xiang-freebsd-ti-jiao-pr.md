...
为了让人们更易贡献，FreeBSD 项目最近开始支持 GitHub 上的拉取请求（pull request，PR）。我们发现使用缺陷跟踪器 Bugzilla 接受补丁会造成许多有用的贡献被忽视并搁置至过期。因此我们建议贡献者首选 GitHub PR 来进行修改，仅把 Bug 放在 Bugzilla。虽然 Phabricator 对开发人员很有用，但我们也发现，在 Phabricator 极易遗漏外部贡献者的更改。除非你和直接让你用 Phabricator 的 FreeBSD 开发人员一起工作，否则请改用 GitHub。GitHub PR 更易跟踪和处理，并且更广泛的开源社区对此更为熟悉。我们希望有更快的决策，更少的更改被放弃，能为所有人提供更佳的体验。

由于 FreeBSD 的志愿者时间有限，本项目制定了标准、规范和政策，以高效利用志愿者的时间。你需要了解这些内容才能提交好的 PR。我们有一些自动化程序来帮助提交者修复常见错误，从而使志愿者可以审查几乎就绪的提交。请理解我们仅接受最有用的贡献，而某些贡献我们无法接受。

接下来，我将介绍：如何将你的更改转换为 Git 分支，如何完善它们以符合 FreeBSD 项目的标准和规范，如何从你的分支创建 PR，以及对审查流程的预期。然后，我将介绍志愿者如何评估 PR，并提供完善 PR 的技巧。
...
在实验性使用 GitHub 接受更改几年以后，FreeBSD 项目不得不引入一些限制，对那些尚未获得 FreeBSD 项目存储库写入权限的人员通过 GitHub 提交的贡献加以限制。这些限制确保了验证和应用修改的志愿者能够最有效地分配他们的时间。因此，FreeBSD 项目无法接受如下内容：

- 过大而无法在 GitHub 上审核的变更
- 注释中的拼写错误
- 运行静态分析程序时发现的更改（除非这些更改包含了针对静态分析程序发现的错误的新测试用例）。对于与我们的测试框架交互不畅的系统部分“显然正确”的修复，可以根据具体情况作出例外处理。
- 理论性的更改，但没有具体错误或者可描述的行为缺陷。
- 未附带前后对比测量以显示改进的性能优化。微小优化通常不值得，因为编译器和 CPU 技术通常会使它们在几年内（甚至更慢）变得过时。
- 有争议的更改。这些需要在 <freebsd-arch@freebsd.org> 或最适合的邮件列表上先行讨论。GitHub 并不适合讨论此类问题。

PR 应该以某种用户可见的方式改进项目。

## 评估标准

- 该变更是否属于 FreeBSD 项目接受的类型？
- 变更的范围、规模是否合适？
- 提交数量是否合理（比如少于 20 个）？
...
从总体来看，提交到 FreeBSD 的过程简单明了，尽管深入到细节，可能会使这种简洁性变得不是那么明显。

![FreeBSD PR 提交流程图](https://freebsdfoundation.org/wp-content/uploads/2024/07/basicflowchart.png)
...
如果你还没有 GitHub 账户，请创建一个。此[链接](https://github.com/join)将指导你完成创建新 GitHub 账户的过程。由于许多人已因其他原因而拥有 GitHub 账户，我们将跳过具体的详情讨论。

下一步是将 FreeBSD 的存储库 fork（复刻）到你的账户中。使用 GitHub 网页是创建复刻和进行解释的最简单方法——因为此操作你仅需执行一次。对复刻的更改不会影响 FreeBSD 存储库。用户可以通过单击“Fork”按钮（如图 1 所示）来复刻存储库。你需要点击突出显示的菜单项“Create a new fork（创建新复刻）”。将打开类似于图 2 的界面。在这里，点击绿色的按钮“Create Fork（创建复刻）”。

![GitHub Fork 菜单示意图](https://freebsdfoundation.org/wp-content/uploads/2024/07/Figure1_witharrows.jpg)

图 1：单击“Fork”旁边的向下箭头后，你将看到一个弹出式窗口，显示创建复刻的对话框。

![GitHub 创建复刻界面](https://freebsdfoundation.org/wp-content/uploads/2024/07/Figure2.png)

图 2：创建复刻，第 2 部分。

在你点击“Create Fork（创建复刻）”后，GUI 将重定向到新复刻的存储库。你可以在常规位置复制克隆版本库所需的 URL，如图 3 所示。

![GitHub 克隆 URL 示意图](https://freebsdfoundation.org/wp-content/uploads/2024/07/Figure3.png)

图 3：复制用于克隆的网址（我以前用旧存储库名称创建的复刻）
...
此外，请阅读我们关于[提交日志消息](https://docs.freebsd.org/en/articles/committers-guide/#commit-log-message)的建议，并在创建提交时遵循它。
...
![GitHub PR 提交表单](https://freebsdfoundation.org/wp-content/uploads/2024/07/figure4.png)

图 4: PR 提交表单。

在字段“Add a Title（添加标题）”中，添加简要描述你所做的工作，以传达修改的本质。保持在十几个词左右，以便易于阅读。如果分支仅有单个提交，使用提交消息的第一行作为此更改的标题。如果有多个提交，则需要总结它们为一个简短的标题。

在字段“Add a Description（添加描述）”中，写下你的更改摘要。如果分支仅包含单个提交，请使用提交消息的正文部分。如果有多个提交，则创建简短摘要，描述解决的问题。解释你做了哪些更改及原因，如果不是显而易见的话。

图 4 中的示例试图解决贝尔实验室和伯克利之间著名的历史争端。这是下文所述存在争议提交的良好示例。这是应当讨论的争议性提交的良好示例。

## 期待什么

提交之后，评估过程开始。项目会运行多个自动检查器。这些检查确保提交的格式和样式符合我们的指南，确保更改可以编译通过。它们将提供反馈，指出在有人查看之前你应进行的修改。其中一些测试需要时间，因此在提交后几个小时再来检查是个好主意。自动测试标记的项目将是我们的志愿者要求你更正的首要事项，因此积极解决这些问题可以为所有人节省时间。

## 回复反馈

若收到反馈，通常需要更改代码。请执行建议的更改。通常这意味着你需要修改部分内容（提交消息或提交本身）。GitLab 提供了关于使用 git rebase 机制的[教程](https://docs.gitlab.com/ee/topics/git/git_rebase.html)。

更改完成后，你需要把更改推送回分支，以便 PR 更新并重新触发反馈循环：
...
最近，恶意行为者攻击了 xz 源代码存储库，插入了某些代码，从而影响了某些 Linux 系统上的 sshd。凭借运气和流程，FreeBSD 未受本次攻击的影响。我们的流程通过多重保护层抵御此类攻击。我们会在代码进入测试前进行审查。仅在提交的代码中未发现明显恶意行为时，我们才会运行自动化测试。开源项目面对日益恶劣的工作环境，一些看似多余的提问往往有其必要。

## 结语

无论你是偶尔微调 FreeBSD 的休闲用户，还是频繁提交更改以至获得提交权限的开发者，FreeBSD 项目都欢迎你的提交。本文尝试涵盖一些基础内容，但更偏向于休闲用户。在线资源将帮助你处理基础之外的情况。

---

**WARNER LOSH** 在 FreeBSD 项目成立前就参与了开源——甚至早于“open source，开源”这个术语的正式定义。最近，他深入探索了 Unix 的早期历史，旨在揭示其丰富而隐秘的遗产。他与妻子和女儿居住在科罗拉多州的一座稻草板房屋中，这座房屋由太阳能、小锅炉（偶尔还用古董计算机）供热。
