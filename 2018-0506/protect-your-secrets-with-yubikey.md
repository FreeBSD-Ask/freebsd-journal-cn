...
可以使用两步验证与 Google、Facebook、GitHub 和 Hotmail 等 Web 服务配合使用。这很有用，因为即使攻击者窃取了第一个认证因素（账户的用户名和密码），他们也无法登录。我们可以通过使用支持 U2F 的 YubiKey 设备来确保这一点。当然，我们必须在所选 Web 服务上启用两步验证。
...
YubiKey 是一个非凡的设备，可用于企业或个人提高安全性。这个小设备支持许多不同的认证方法，可与许多流行的 Web 服务以及 SSH 等程序配合使用。它还允许我们利用一些不支持 2FA 的工具的不完美之处。它可以用作第二因素或保存主密码。它是手机应用等其他解决方案的有趣替代品。YubiKey 还允许我们无痛地更改密码，无需任何记忆。
