...
NFSv4.1 RFC 规定 `OPEN_SHARE_ACCESS_WANT` 位可以设置在 OpenDowngrade 的 `share_access` 参数中，并且基本被忽略。它还将错误从 `NFSERR_BADXDR` 改为 `NFSERR_INVAL`，因为 NFSv4.1 RFC 规定如果设置了虚假位，应返回此错误。（NFSv4.0 RFC 没有为这种情况指定任何错误，因此 NFSv4.0 也可以更改错误回复。）
