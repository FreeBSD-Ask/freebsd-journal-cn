...
是的，我们为服务自己的流量对 FreeBSD 做了大量额外改动。我们也确实在努力把它们都上游。不过过程并不容易。开源代码的质量标准实际上高于生产代码。生产环境中你只关心自己运行的架构，不关心其他架构；只关心自己的工作负载，只关心你使用的那部分操作系统。要把某些改动上游，我们必须照顾所有其他平台、工作负载、API 和标准的潜在使用者。同时，让我们的代码满足开源标准的同时不能让我们自己的场景性能下降。有时同时满足内部和开源两方面的要求会相当困难。
