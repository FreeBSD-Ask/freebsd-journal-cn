...
简单的视频游戏对我来说和玩它们一样有趣。我意识到计算机将是我未来的乐趣和生计。我享受阅读大量书籍。我的父亲是文学教师，我们的家庭藏书丰富。我还学习音乐并在学校乐队弹吉他。我必须承认，计算机学习把音乐从我生活中赶走了，但我对好音乐的品味一直保留着。
...
对于想成为 FreeBSD 提交者的人：你可以做到！帮助我们改进 FreeBSD 的代码、文档或 Ports。在享受乐趣的同时提升自己，与其他贡献者和提交者交流，主动提供帮助也向他们求助，你会成功的。我们需要新人的加入！”——EUGENE GROSBEIN

家中桌面安装了 3.2-RELEASE，并开始在 ISP 环境（2.2.8-RELEASE）的工作中使用 FreeBSD。那是我开始通过 GNATS 提交问题报告为 FreeBSD 项目做贡献的时候（PR 15301 是我的第一个）。

Zverev 给我提供了升级到 Ports 提交权限的机会并成为我的导师。我们在 IRC 频道上讨论了这件事，我欣然同意。
...
你是怎么成为提交者的？• 2001 年，我创建了第一个 Port，`databases/libudbc`（现已退役），用于 Openlink Universal DataBase Connectivity SDK，我用它将基于 FreeBSD 的 Web 服务器连接到运行在 VAX/OpenVMS 下的 DEC RDB 数据库。从那时起，我就一直在 FreeBSD“额外贡献者”名单上。与此同时，我继续在多个 ISP 和其他商业环境中使用 FreeBSD。我甚至一度运行 64 位 FreeBSD 4.x/alpha。此后，我提交了约 340 个 PR，尽可能提交补丁。我想我未关闭的报告数量达到了临界质量，因此 Vsevolod Stakhov 和 Andrej

DRU LAVIGNE 是 FreeBSD 项目的文档提交者，BSD Certification Group 主席。

TM

基于浏览器的版本（DE）现以 PDF 形式提供查看，支持打印选项。

JOURNAL

全新查看与打印选项

基于浏览器的 DE 格式为订阅者提供与应用相同的功能，但允许通过你喜欢的浏览器查看 Journal。与应用不同，你可以将 DE 作为 PDF 页面查看并打印。

随着时间推移，Unix 操作系统演化出多种启动程序并影响其执行的机制。为更好地理解这一点，让我们快速浏览在典型 FreeBSD 系统上运行的不同类型程序。

计算机首次启动时，`rc(8)` 机制启动称为“服务”的程序，这些服务可以执行一次性系统初始化任务，或运行后台守护进程。这些服务也可以在计算机运行时停止和启动。计算机运行的每一分钟，`cron(8)` 守护进程都会唤醒并启动计划任务。Cron 还支持在特定时间间隔后运行 `at(1)` 作业。它也可以在系统负载低于某阈值时运行 `batch(1)` 作业。还有许多其他启动程序的机制。`inetd(8)` 在创建传入套接字时启动程序。`nice(1)` 以修改后的优先级运行程序。`chroot(8)` 以备用根目录启动程序。`service(8)` 通过“监管”进程启动程序，该进程可在程序崩溃时重启程序。像 `jail(8)` 这样的 Jail 管理器在 Jail 环境中启动程序。像 `iohyve(8)` 这样的虚拟机管理器启动执行

如需订阅或获取过刊以及了解基金会的其他事务，请访问

$19.99

DE 与应用一样是个人产品。每次发布新期刊时你都会收到邮件通知。
