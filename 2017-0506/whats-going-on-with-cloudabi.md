...
FreeBSD Journal 2015 年 9/10 月刊刊登了一篇

关于 CloudABI 的文章，这是我同年早些时候开始工作的

一个开源项目。距离那篇文章发

表已经过去相当一段时间了，让我们来看看

此期间发生的一些进展以及接下来的计划。但首

先，回顾一下 CloudABI 是什么以及使用它是什么样子。
...
ZFS 专家让他们的服务器 现在你也可以。获取一本......

作者：Brooks Davis、Robert Norton、Jonathan Woodruff 和 Robert N. M. Watson

选择电子书、印刷版或组合版。你将学到：

- 使用启动环境，让风险最高的系统管理任务变得乏味。
- 将文件系统权限委派给用户。
- 用 Jail 容器化 ZFS 数据集。
- 快速高效地在机器间复制数据。
- 从镜像中分出层。
...
无论你管理单台小型服务器还是国际数据中心，都用 FreeBSD Mastery: Advanced ZFS 简化你的存储。立即获取！
...
过去几年，集群管理系统领域发生了大量发展。这些系统允许你将一大组服务器视为单一计算资源池，在其上调度作业。一个日益流行的是由 Google 设计、通过 Linux 基金会新成立的云原生计算基金会（CNCF）资助的 Kubernetes。Kubernetes 可以运行任何已打包为容器（使用 Docker、rkt 等）的软件。我观察到的 Kubernetes 使用中的一个问题是，它有一些怪癖，源于它必须能够运行非依赖注入的软件。例如，由于容器中运行的程序可自由绑定任意网络端口，集群中每个作业（“Pod”）必须
...
Dell 现已成为 Dell Technologies 家族的一部分。因此，你可以在从存储和大数据到物联网的各个领域留下你的印记。

在 Dell.com/careers 申请
