...
内存自动管理，无需手动分配或释放，避免相关错误。作为使编程更安全的设计目标之一，Swift 在很大程度上不向开发者暴露指针，但需要时也可使用。它要求变量使用前先声明，避免脚本语言中常见的隐式声明问题。类型可推断以简洁，也可在需要安全与清晰处显式声明。数据类型严格以策安全，但类型也可轻松转换以保灵活。
