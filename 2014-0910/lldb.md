...
此后，LLDB 已超越了 Apple 项目的范畴，来自 Intel 和 Google 等公司内部的开源团队，以及 FreeBSD、Debian 和其他独立开源项目都做出了重要贡献。多人参与了 LLDB 的 FreeBSD 移植，我们的计划是让它成为 FreeBSD 基本系统的标准调试器。
...
LLDB FreeBSD 移植的持续开发工作直接在 LLDB 仓库中进行。它目前在 amd64 架构上运行良好，支持基于实时和核心转储文件的用户态调试。MIPS 架构也存在核心转储调试支持。结合 Google 正在进行的 CPU 支持工作和 FreeBSD 社区正在开展的工作，我们预计将支持 x86、ARM、MIPS 和 PowerPC CPU 架构的 64 位和 32 位版本。
