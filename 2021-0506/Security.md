...
在进行全新安装时，决定是否启用硬盘加密非常重要——特别是因为后续添加加密可能会比较困难。磁盘加密的目的是保护存储在系统上的数据。我们在计算机上存储了大量个人和商业数据，大多数时候，我们不希望这些数据被未经授权的人读取。我们的笔记本电脑可能会被盗，如果是这样，就应该假设小偷可能会访问几乎全部的文档、照片，以及我们通过浏览器登录的所有页面。有些人也可能尝试篡改或读取我们的数据，当我们远离电脑时尤其如此。

我们建议加密所有系统——特别是服务器或个人电脑。硬盘加密的成本相对较低，带来的好处也很明显。
...
基于 Geom 的磁盘加密（GBDE）是 FreeBSD 中最早的磁盘加密机制。但目前 GBDE 被开发者忽视。GBDE 仅支持 AES-CBC 加密算法，使用 128 位密钥长度，并且每次写入时使用不同的密钥，这可能会带来一定的 CPU 开销。它还需要存储每个扇区的密钥，这增加了一些磁盘空间开销。出现了其他更高效、更灵活的加密替代方案后，GBDE 对开发者和用户来说不再有吸引力。

OpenZFS 原生磁盘加密是 FreeBSD 中最新的磁盘加密机制，首次出现在 13.0-RELEASE 中。OpenZFS 支持 AES-CCM 和 AES-GCM 加密算法，密钥长度可以是 128、192 或 256 位。在 OpenZFS 的加密中，并非所有数据都会被加密。数据集和快照、池布局或数据集属性等将以未加密的方式存储。通常这些数据可能并非机密，但用户应当意识到这一点。OpenZFS 磁盘加密的另一个问题是，FreeBSD 不能从加密的数据集启动。

如果用户仅希望加密少量数据集并保护系统的某些部分，OpenZFS 原生磁盘加密是一个可靠的选择。OpenZFS 原生加密的一个重要优点是，可以在系统安装后启用。唯一的要求是使用 ZFS，用户可创建新的加密数据集。
...
![FreeBSD 安装程序 GELI 加密配置](../png/2021-0506/Security-01.png)
...
ZFS 支持多种校验和算法，默认使用 SHA-256。用户可以决定启用还是禁用完整性检查，强烈建议保持启用状态。凭借数据完整性，我们可以确保数据未被篡改。

## 3. ZFS setuid 和 exec 属性

除了数据完整性，我们还可以通过 ZFS 增加一些额外的保护。我们喜欢在不需要 setuid 的数据集上禁用该功能。ZFS 的 setuid 属性控制文件系统中 setuid 位是否生效。当运行 setuid 二进制文件时，它会模拟二进制文件所有者的身份。setuid 非常有用，但同时也非常危险，因此我们建议限制其使用，并在大多数数据集上禁用此属性——除了包含系统 **/bin** 和 **/sbin** 目录的数据集。

另一个有吸引力的属性是 exec。此属性能让我们控制是否允许执行数据集中的程序。我们喜欢在 **/tmp** 和下载目录禁用它。大多数时候，我们将随机文件移到这些目录或下载文件，我们不希望误执行这些文件。
...
sudo(8) 是一款应用，让你可以控制以 root 或系统中其他用户身份运行命令的访问权限。系统管理员可以创建特权命令列表，用户可以以其他用户身份执行这些命令。与 su(1) 不同，sudo(8) 不需要 root 或其他用户的密码来运行。当用户想要执行具有提升权限的命令时，会要求用户输入自己的密码。这限制了系统管理员之间密码的共享。我们可以通过 FreeBSD 的二进制包管理轻松安装 sudo(8)：

```sh
# pkg install sudo
```

通过 sudo(8) 调用的活动会被记录。这使谁做了什么操作可被追溯。这也是为什么我们应避免以不同用户身份启动或登录。

请记住，不要过度复杂化 sudo 配置，保持尽可能简单，绝不要通过 sudo(8) 授予用户可自行修改的脚本/程序的执行权限。被忽视的 sudo 条目可能成为攻击者易用的攻击向量。
...
只需确保有单用户模式启动选项或其他恢复方法，以便在需要解锁 root 时使用。

通过日常使用 sudo(8)——而不是使用 root shell——用户还可以避免在离开未锁定的计算机时出现问题，因为此时有人可能会利用 root shell 安装恶意软件。sudo(8) 通过在一段时间不活动后要求用户输入密码来缓解这个问题。

## 5. 备份

世界上有两种人——做备份的人和将来会做备份的人。不幸的是，第二类人会通过失去数据的痛苦方式才认识到备份的重要性。关于勒索软件和云服务提供商的故事告诉我们，很多人仍然抵触做备份。预防胜于治疗——备份也是如此。用户通常直到为时已晚才明白备份的重要性。如果你从这篇文章中学到的唯一一课——请记得执行备份。
...
另一个需要记住的教训是定期升级我们的操作系统。这是每个人都在谈论的教训，但不知为何常常被遗忘。
...
我们选择的文件系统是 ZFS，因此在进行升级时使用启动环境是个好方法。这使我们可以在系统出现问题时轻松回滚到先前的版本。

## 7. 安全加固

在安装新的 FreeBSD 实例时，靠后的某个菜单是关于操作系统加固的。一些选项会对 FreeBSD 进行小幅调整，从而略微提高安全性。我指的是像隐藏进程 UID 和 GID、在系统启动时清空 tmp 目录以及随机化进程标识符等设置。

默认情况下，所有的加固选项都是禁用的。我们建议启用这些选项。

![FreeBSD 安装程序安全加固选项](../png/2021-0506/Security-02.png)
...
用户还可以将这些条目添加到 **/boot/loader.conf** 中，以便在启动时启用它们。

## 总结

在本文中，我们介绍了七种提高 FreeBSD 系统安全性的方法。在 FreeBSD 中使用 ZFS 非常流行，但减少数据集执行 setuid 和 exec 文件的能力并不常见——而用户可以从中受益。我们希望全盘加密、备份和 sudo(8) 的使用已经成为标准，但总是值得提醒大家使用它们。我们还讨论了 FreeBSD 配置中的一些小调整，这些调整可以随机化或隐藏一些系统信息。
...
**MARIUSZ ZABORSKI** 目前是 4Prime 的安全专家，自 2015 年起拥有 FreeBSD 提交权限。他的主要兴趣领域是操作系统安全和底层编程。过去，他曾在 Fudo Security 工作，领导开发 IT 基础设施中最先进的 PAM 解决方案。2018 年，Mariusz 组织了波兰 BSD 用户组。在业余时间，他喜欢在 [https://oshogbo.vexillium.org](https://oshogbo.vexillium.org) 上写博客。
