...
这将执行一次干净的、单线程的内核构建。通常最好利用尽可能多的 CPU，因此可以考虑将 `-j $(sysctl -n hw.ncpu)` 添加到 **make(1)** 的参数中：这将并行运行与系统中的 CPU 数量相等的构建任务。从特定源路径构建内核后，通常不需要每次测试更改时都重建每个单独的源文件——增量重建就足够了。要请求增量构建，可以将 `-DKERNFAST` 标志添加到 **make(1)** 命令中：
