...
TCP 备选栈框架试图解决这一问题，将栈模块化，使任何人都能提出新的 TCP 栈，并让两个（或更多）栈同时共存于同一台机器上，服务不同的连接。代码经过模块化，不同的 TCP 操作（如输入或输出处理、段处理、定时器处理和套接字选项解析）被细分为不同的函数块，并通过函数指针访问。因此，开发者可以设计一个采用新方式处理输入或输出的备选栈，同时仍使用默认栈的其余功能。可以通过套接字选项为每个连接选择栈，这提供了在试验各种 TCP 特性时进行真正 A/B 测试的能力。
...
下面是一个简短示例，服务端将 `mytcp` 设为监听套接字上使用的栈。它接受的每个新连接都会以 `mytcp` 作为其 TCP 栈。
...
客户端可以发起 TFO 连接，直到 cookie 有效；其有效期可在服务端配置。因此，如果客户端向同一服务器发起大量短生命周期连接，该特性对降低整体延迟大有帮助。
...
数据中心 TCP（DCTCP）是一种用于数据中心的拥塞控制机制。它使用显式拥塞通知（ECN）来估计拥塞的程度/规模，而不仅仅检测是否发生拥塞。
...
当发送数据包两次因重传超时（RTO）失败时，它会尝试检测连接路径上是否遭遇黑洞。方法是将最大段大小（MSS）降至一个预设值。如果连续发送失败，则继续降低 MSS，直到达到配置的最小 MSS 值。如果使用较低的 MSS 能发送数据包，说明路径上确实存在黑洞。但如果即便使用配置的最小 MSS 值仍无法发送数据包，则不是黑洞，而是真正的拥塞导致丢包，此时将 MSS 恢复为原始值。

该特性的另一个重要部分是在成功传输后，以递增的 MSS 值向上探测网络。然而这部分目前在 FreeBSD 中尚未实现，因此该特性默认禁用。

在许多拥塞控制机制中，拥塞窗口的增长取决于 MSS 大小，因此遭遇黑洞的连接可能经历拥塞窗口增长缓慢，从而更慢地利用路径容量。
...
在此短生命周期连接的改进之前，近 80% 的接收数据包在持有独占写锁的情况下处理。这意味着只有一个 CPU 核心能处理接收数据包，其余核心必须等待锁释放。现在新增了一个 INP_LIST 锁，保护全局 inpcb 列表的修改。这允许在输入处理等关键路径上使用 INP_INFO_RLOCK（读锁），只在真正需要时偶尔使用 INP_INFO_WLOCK（写锁），即在稳定时遍历全局 inpcb 列表。
...
发送挑战 ACK 会使真正的发送方生成一个与我们期望的精确序列号匹配的 RST。基本上，有了这一机制，攻击者很难伪造能让我们断开连接的有效 SYN/RST。某些中间设备或损坏的 TCP 栈可能仍以我们不期望的 SEQ 号的 RST 数据包响应挑战 ACK。在这种情况下，由于该特性，我们可能看到大量虚假的挑战 ACK 和 RST 来回发送。应对挑战 ACK 进行限流以缓解此问题，使我们在每个时间段内只发送有限数量的挑战 ACK。例如，5 秒内只发送 10 个挑战 ACK。允许的时间和 ACK 数量都应是可调参数。这在 FreeBSD 中尚未实现。
...
现在，通过 DTrace（一种动态跟踪框架）探针点可实现相同功能，无需重新编译内核或用户空间应用程序。**$src/share/dtrace/tcpdebug** 是一个示例脚本，也是如何使用该特性的好例子。

---

**HIREN PANCHASARA** 在内核代码中摸爬滚打了几年，最近更多地在网络和 TCP 领域。他在一切可能的设备上运行 FreeBSD，并希望通过指导等方式将越来越多的新手带到 FreeBSD。
