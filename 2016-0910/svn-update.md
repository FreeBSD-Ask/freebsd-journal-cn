...
EC2 中 `boot_multicons="YES"` 切换为 `boot_multicons="YES"`。这使运行在 Elastic Compute Cloud 中的 FreeBSD 实例能够利用 Amazon 近期推出的一项 API（<https://aws.amazon.com/blogs/aws/ec2-instance-console-screenshot/>），对模拟的 VGA 设备进行截图。

## Xen blkfront 驱动

在 EC2 上运行时，Xen blkfront 驱动默认启用 indirect segment I/O。得益于 EC2 的改进，某些 EC2 实例上曾经存在的性能损失不复存在，启用该特性现在能稳定带来约 20% 的吞吐量提升，且延迟相同或更低。

## MFC r302901

MFC r297187（<https://svnweb.freebsd.org/base?view=revision&revision=302901>）

Intel PRO/1000 千兆以太网适配器现在支持 TCP/IPv6 和 UDP/IPv6 的校验和卸载，以及 SCTP/IPv6 的 SCTP 校验和卸载。

## MFC r302800

MFH r297023（<https://svnweb.freebsd.org/base?view=revision&revision=302800>）

`kldstat`(8) 现在能够以合适的格式打印模块特定的信息。除其他用途外，这使我们能够查询动态加载的、向量号动态分配的系统调用的 syscall 编号。
...
mlx5en 驱动新增 SR-IOV 客户机支持。这补全了在通过 SR-IOV 提供硬件访问的虚拟机内使用 mlx5en 驱动进行设备设置所需的缺失部分。
