...
随着企业及其客户日益认识到通信隐私的价值，传输层安全（TLS）[2] 的重要性持续增长。Netflix 在 2015 年宣布，将开始对其流媒体服务的视频和音频播放会话进行加密，以帮助用户保护观看隐私 [3]。启用加密会给 OpenConnect 服务带来显著的计算开销，因此我们持续探索并实现优化 TLS 批量加密的新方法，以降低资本和运营成本。
...
对 OpenCrypto 框架 AES-CBC 密码的性能测试表明，其性能低于 OpenSSL 的等效实现。我们还需要调查 AES-GCM 性能，但发现截至 2015 年初，OpenSSL 1.0.1 没有 AES-NI 优化的实现。对替代方案的寻找使我们发现了 BoringSSL [6]，它有一个性能良好的 AES-GCM 的 AES-NI 实现。
