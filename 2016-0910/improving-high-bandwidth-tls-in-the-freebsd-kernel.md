...
密码块链接（CBC）常用于实现 TLS 会话的批量数据加密，因为它研究充分且相对容易实现。但它计算开销高，因为明文数据必须处理两次：一次生成加密输出，一次生成验证加密数据完整性的 SHA 哈希。基于 Galois/Counter Mode（GCM）[5] 的 AES-GCM 密码提供足够的数据保护，且不需要明文处理两次。它包含在 TLS 1.2 及更高版本中，在所有现代版本的 OpenSSL 及其衍生版本中可用。解密的计算开销也很低，加之其广泛可用，对客户端和服务器平台都有吸引力。

我们决定将 TLS 平台过渡为优先使用 GCM，只对无法升级以支持 GCM 的原始客户端回退到 CBC。我们估计，一旦 TLS 推广到整个网络，只有小部分播放会话会使用 CBC。

## 密码实现

对 OpenCrypto 框架 AES-CBC 密码的合成性能测试表明，其性能低于 OpenSSL 的等效实现。我们还需要调查 AES-GCM 性能，但发现截至 2015 年初，OpenSSL 1.0.1 没有 AES-NI 优化的实现。我们寻找替代方案时发现了 BoringSSL [6]，它为 AES-GCM 提供了性能良好的 AES-NI 实现。
...
ISA-L 的一个缺点是它用 YASM 方言编写，与 FreeBSD 中的 GCC 和 LLVM 工具链汇编器不直接兼容。这要求我们对内核 makefile 基础设施做基本修改，允许将 YASM 作为外部汇编器用于所需源文件，如下：
...
我们在 2015 年的初始工作 [4] 使用 FreeBSD 内置的 AES-NI 实现和 Open Crypto Framework（OCF）执行批量加密。我们上一篇论文中的结果显示了较小的性能提升，但远未达到我们希望获得的成果。我们知道有几个方面可以改进，包括：
...
通过最新工作，总体性能提升了多达 30%，比我们最初的结果有巨大改进。尚未探索的是使用卸载卡来协助加密所有数据。我们的问题是，是否能在我们的硬件设计中使用卸载卡，使其消耗的 CPU 和 PCIe 带宽少于直接运行 AES-NI 指令本身。随着新一代 Intel CPU 的推出，向辅助卡输入数据并收集加密结果的开销可能会超过 AES-NI 指令本身的开销。

我们还被迫搁置 OpenCrypto 框架以实现某些优化。类似的优化可能对内核中的其他加密消费者有用，因此在我们将代码上游到 FreeBSD 的过程中，将探索扩展 OCF API 的方式。

软件加密仍然要求 CPU 处理数据页，导致 CPU 缓存污染。这些数据几乎不会被重用，因此缓存是浪费的，并且不必要地驱逐了其他可重用的系统数据。我们目前正在研究 Intel CPU 缓存控制特性的细粒度使用，以限制加密过程中放入末级缓存的数据量。我们还在与 Intel ISA-L 团队合作，开发使用非缓存加载/存储汇编操作码在密码中移动数据的例程。

## 致谢

作者感谢 Intel ISA-L 团队主动联系我们，协助实现性能目标，感谢 David Wolfskill 和 Warner Losh 的支持和评审。还要感谢整个 Netflix OpenConnect 团队为这项工作提供了出色的环境。

## 补遗

本文最初于 2016 年 3 月在 AsiaBSDCon 发表后不久，Intel 公开发布了 E5-v4（Broadwell Xeon）系列 CPU。我们当时已在 TLS 工作负载下评估这些 CPU 的预发布样品，但由于硬件的预发布性质，无法在论文中包含结果。我们还开始尝试使用 Mellanox ConnectX-4 100Gb 以太网适配器替代 40Gb 适配器，以及 HGST SN100 NVMe 驱动器替代 SATA SSD 存储。

2697-v4 CPU 将 CPU 核心数提升至 16，比 v3 前代增加 2 个核心。更重要的是，v4 系列新增了缓存分配技术（CAT），允许操作系统对 L2 和 L3 缓存中数据页的保留进行细粒度控制。这很有吸引力，因为我们的工作负载下通过 CPU 的大部分数据是 TLS 数据，一旦加密阶段结束就永远不会被 CPU 直接检查，通常也不会被重用。将这些数据标记为不可缓存意味着有限的缓存资源可用于 OS 数据和控制结构，而不是一次性的 TLS 数据。支持 CAT 策略只需对内核中的 TLS 套接字层进行直接的代码修改。结果见表 II。

| | 不带 SSL | | 带 SSL | | 带 SSL + CAT | |
| :--: | :--: | :--: | :--: | :--: | :--: | :--: |
...
这些结果基于表 I 的数据，显著区别在于得益于升级到 100Gb 网络接口，CPU 现在是性能的限制因素，而不是网络端口带宽。表中未显示的一个重要数据点是，v4 CPU 也在用户空间 OpenSSL 且无内核数据路径的基线配置下进行了测试。在该配置中，CPU 满载前仅达到 26Gbps。因此，基线与优化配置之间的差异代表 v4 CPU 上性能提升 136%，相比 v3 CPU 基线提升 106%。我们将相对于 v3 的较低增益归因于额外的 CPU 核心造成了更多的资源和缓存争用。

结果表明，设计和部署运行 FreeBSD 的系统是可行的，该系统可以使用适度的硬件组件将 2x40Gbps 或 1x100Gbps 网络附件填充到合理容量，且全部为 TLS 流量。性能研究在 2016 年继续进行，相关结果将在后续论文中分享。

---

**Randall Stewart** 现就职于 Netflix Inc.，担任高级软件工程师。当前职责包括在 Netflix OpenConnect 设备中优化和增强 FreeBSD 的网络栈。此前 Stewart 在 Cisco Systems 担任杰出工程师。在之前的职业生涯中，他还曾在 Adara、Motorola、NYNEX S&T、Nortel 和 AT&T Communication 工作。整个职业生涯中，他专注于操作系统开发、容错和呼叫控制信令协议。Stewart 也是 FreeBSD 提交者，负责 FreeBSD 中的 SCTP 参考实现，并致力于 FreeBSD TCP 栈的增强和扩展。

**Scott Long** 自 2011 年起在 Netflix Inc. 担任高级软件工程师，专注于 FreeBSD 性能和系统架构。此前他就职于 Yahoo、Adaptec 和若干科技创业公司。Long 自 2000 年起成为 FreeBSD 提交者，自 1993 年起成为用户。工作之余，他喜欢在科罗拉多州与家人一起飞行、徒步和滑雪。
