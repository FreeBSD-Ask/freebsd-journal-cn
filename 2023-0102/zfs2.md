...
现在，数据集 `example/compressed` 是 ZFS 压缩文件系统。要禁用压缩，可以使用：
...
验证 `example/compressed` 是否不在输出的挂载文件列表中。可以通过 `zfs mount` 重新挂载文件系统：
...
ZFS 数据集的创建方式与其他文件系统一样，以下示例创建了名为 `data` 的新文件系统：

```sh
# zfs create example/data
```

使用 `df` 查看数据和空间使用情况（为清晰起见，省略了部分输出）。
...
由于这些文件系统建立在 ZFS 之上，它们从同一个存储池中获取存储空间。这样就不需要其他文件系统所依赖的卷和分区。
...
RAID-Z 池需要三块或更多磁盘，但在磁盘发生故障时可以提供数据丢失保护。由于 ZFS 池可以使用多个磁盘，因此文件系统设计本身就支持 RAID。
...
可以创建文件系统快照，以便以后回滚，快照名称用黄色标记，可任意命名：

```sh
# zfs snapshot storage/home@11-01-22
```

ZFS 创建数据集的快照，用户可备份文件系统以便将来回滚或恢复数据。
...
每个软件 RAID 都有一种监控其状态的方法。使用以下命令查看 RAID-Z 设备的状态：
...
ZFS 使用校验和验证存储数据的完整性，这些数据校验和可以被验证（称为清理）以确保存储池的完整性：  
...
虽然本入门指南未涉及 ZFS 管理，但你可以参考 `zfs(8)` 和 `zpool(8)` 以获取更多 ZFS 选项。  

## 更多资源  

虽然通过本指南创建的非冗余和 RAID-Z 池可以在大多数用例中工作，但更复杂或专业的系统可能需要进一步的 ZFS 管理和设置。本指南仅触及了 ZFS 功能的表面，因为它是一个极其强大且可定制的文件系统。OpenZFS Wiki 提供了有关安装、ZFS 系统管理和手册页的广泛文档。如果因系统架构需要调优，可以在 OpenZFS 和 FreeBSD Wiki 页面上找到 ZFS 调优指南。
