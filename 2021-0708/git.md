...
首先，必须下载源代码树。这是一个相当简单的过程。第一步：克隆源代码树。这将下载整个源代码树。有两种方式下载。在默认情况下，Git 会进行深度克隆，这符合大多数人的需求。然而，在某些情况下，你可能希望进行浅克隆。

### 1.1 分支名称

新 Git 仓库中的分支名称与 Subversion 中的名称类似。对于稳定分支，它们是 stable/X，其中 X 是主版本号（如 12 或 13）。新仓库中 -CURRENT 的开发分支是 main。

注意：如果你省略下面的分支选项 `-b` ，则默认使用 main 分支。
...
幸运的是，git 提供了命令 `git bisect` 。这里是如何使用它的简要概述。更多信息，我建议查阅 <https://git-scm.com/docs/git-bisect> 获取更多细节。该 man 页面对于描述可能出错的情况、在修订版无法构建时该怎么办、何时想使用其他术语（如 good 和 bad）等问题非常有用，这些内容在这里不会详细讲解。
...
保留本地更改（尤其是微小的更改）最简单的方法是使用 `git stash`。在最简单的形式下，你使用 `git stash` 来记录更改（这会将它们推送到堆栈中）。大多数人使用此方法在更新树之前保存更改，正如上面所述。然后，他们使用 `git stash apply` 将更改重新应用到树中。`stash` 是一组可以通过 `git stash list` 查看更改的堆栈。`git-stash` 手册页（[https://git-scm.com/docs/git-stash](https://git-scm.com/docs/git-stash)）包含了所有详细信息。
...
假设这包括了上游对 terminfo 的根本性更改以及选项名称的更改。当你更新时，你可能会看到类似这样的内容：
