...
可插拔身份验证模块（PAM）也许是 FreeBSD 中最不被理解、但部署最广泛的部分。每个 FreeBSD 系统都通过 OpenPAM 套件使用 PAM。大多数系统管理员都不会去修改它；如果他们不得不修改，只能照着网上那些看似还算合理的隐晦帖子操作。PAM 比看起来要简单，但这种简单性可能让你无意中搞出复杂问题。使用 PAM 的关键就在于避免这种情况发生。
...
**MICHAEL W LUCAS** 是《Absolute FreeBSD》、《FreeBSD Mastery: Jails》以及其他四十八本书的作者，其中一本便是（鼓点响起）《PAM Mastery》。了解更多信息，请访问 [https://mwl.io](https://mwl.io)。
