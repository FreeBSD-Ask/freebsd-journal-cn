...
系统根目录是系统中共享二进制文件在备用目录中的副本。系统根目录通常可以包含完整的系统安装映像。如果系统根目录包含编译器使用的头文件和库，交叉编译器就能用系统根目录为备用系统编译二进制文件。GCC 和 clang 都用标志 `--sysroot` 指示编译器在系统根目录中查找头文件和库。在 GDB 中，通过将变量 `sysroot` 设为系统根目录的路径来指定。GDB 会在这个系统根目录下而非主机的根文件系统中查找共享库和内核模块。
